...
\begin{document}
    ...
    \section{Introduction}
        ...
        \subsection{Periodontitis}
            Periodontitis is an inflammatory condition which effecting periodontium that surround and support teeth. Major components of periodontitis are clinical attachment loss and bone loss \cite{periodontitis1}. Previous study found risk factors of periodontitis such as smoking, diabetes, genetic factors and host response \cite{periodontitis2}.
    ...
    \section{Methods}
        \subsection{QIIME2 Workflow}
            ...
            \subsubsection{Merging Denoising and Taxonomy Classification}
                After denoising and taxonomy classification steps, some different IDs (ASVs or OTUs) have been identified as same taxonomy. In that case, the different IDs will be merged into one taxonomy (Figure \ref{fig:merging}).
    ...
    \section{Results}
        ...
        \subsection{Random Forest Classifier with Every Class}
            As figure \ref{fig:denosing-workflow} and figure \ref{fig:taxonomy-workflow}, there are six combinations. Thus, classification algorithm is carried out on these six combinations (Figure \ref{fig:RF-whole-workflow}). Among these six combinations, Deblur and HOMD pair gives the best result in terms of balanced accuracy. Features which are ordered by importance are in table \ref{tb:RF-every-Deblur-homd}. Also, five metrics by feature count are shown as figure \ref{fig:RF-every-metrics-Deblur-homd}; then, the highest value of balanced accuracy is 0.778 with using 13 taxa. Moreover, violin plots from the two most important taxa, \textit{Porphyromonas gingivalis} and \textit{Actinomyces}, are in figure \ref{fig:RF-every-important-Deblur-homd}.
        ...
        \subsection{Random Forest Classifier with Merging (Healthy+Early) Classes}
            As figure \ref{fig:denosing-workflow} and figure \ref{fig:taxonomy-workflow}, there are six combinations. However, there is no statistically significant difference between Healthy and Early classes. Thus, classification algorithm is carried out on these six combinations with merging Healthy and Early classes. Among these six combinations, DADA2 and HOMD pair gives the best result in terms of balanced accuracy. Features which are ordered by importance are in table \ref{tb:RF-HE-DADA2-homd}. Also, five metrics by feature count are shown as figure \ref{fig:RF-HE-metrics-DADA2-homd}; then, the highest value of balanced accuracy is 0.938 with using 11 taxa. Moreover, violin plots from the two most important taxa, \textit{Porphyromonas gingivalis} and \textit{Actinomyces}, are in figure \ref{fig:RF-HE-important-DADA2-homd}.
        ...
        \subsection{Random Forest Classifier with Merging (Moderate+Severe) Classes}
            As figure \ref{fig:denosing-workflow} and figure \ref{fig:taxonomy-workflow}, there are six combinations. However, there is no statistically significant difference between Moderate and Severe classes. Thus, classification algorithm is carried out on these six combinations with merging Moderate and Severe classes. Among these six combinations, Deblur and HOMD pair gives the best result in terms of balanced accuracy. Features which are ordered by importance are in table \ref{tb:RF-MS-Deblur-homd}. Also, five metrics by feature count are shown as figure \ref{fig:RF-MS-metrics-Deblur-homd}; then, the highest value of balanced accuracy is 0.777 with using 28 taxa. Moreover, violin plots from the two most important taxa, \textit{Porphyromonas gingivalis} and \textit{Actinomyces}, are in figure \ref{fig:RF-MS-important-Deblur-homd}.
        ...
        \subsection{Random Forest Classifier with Merging (Healthy+Early) \& (Moderate+Severe) Classes}
            As figure \ref{fig:denosing-workflow} and figure \ref{fig:taxonomy-workflow}, there are six combinations. However, there are not statistically significant difference between (Healthy and Early) classes and between (Moderate and Severe) classes. Thus, classification algorithm is carried out on these six combinations with merging (Healthy and Early) classes and (Moderate and Severe) classes. Among these six combinations, DADA2 and SILVA pair gives the best result in terms of balanced accuracy. Features which are ordered by importance are in talbe \ref{tb:RF-HEMS-DADA2-silva}. Also, five metrics by feature count are shown as figure \ref{fig:RF-HEMS-metrics-DADA2-silva}; then, the highest value of balanced accuracy is 0.934 with using 15 taxa. Moreover, violin plots from the two most important taxa, \textit{Porphyromonas gingivalis} and \textit{Treponema denticola}, are in figure \ref{fig:RF-HEMS-important-DADA2-silva}.
            ...
            \begin{figure}[p]
                ...
                \end{array}$
                \caption{Most and Second Most Important Features with DADA2 and HOMD for Merging (Healthy+Early) \& (Moderate+Severe) Classes}
        ...
        \subsection{Random Forest Classifier with Healthy Class and Early Class Only}
            ...
            \begin{table}[p]
                ...
                \csvreader[tabular=cp{10cm}l, no head, column count=3, table head=\hline, late after first line=\\\hline, table foot=\hline]{csv/RandomForest/HE-DADA2-homd.txt}{}{\csvlinetotablerow}
            \end{table}
