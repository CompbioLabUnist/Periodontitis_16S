...
\begin{document}
    \begin{tikzpicture}
        \node[decision] (node00) {Taxnomy};

        \node[cloud, below of=node00, left of=node00] (node01) {Greengenes};
        \path[line] (node00) -- (node01);
        \node[cloud, below of=node00, right of=node00] (node02) {SILVA};
        \path[line] (node00) -- (node02);

        \node[block, below=1cm of node01] (node03) {Kingdom -- Species};
        \path[line] (node01) -- (node03);
        \node[block, below=1cm of node02] (node04) {Domain -- Genus};
