...
def aggregate_confusion_matrix(confusion_matrix: numpy.ndarray, derivation: str = "") -> float:
    ...
    with numpy.errstate(divide="ignore"):
        return list(filter(lambda x: (x[0] == derivation), zip(derivations, (TP / (TP + FN) if (TP + FN) else numpy.nan, TN / (TN + FP) if (TN + FP) else numpy.nan, TP / (TP + FP) if (TP + FP) else numpy.nan, TN / (TN + FN) if (TN + FN) else numpy.nan, FN / (FN + TP) if (FN + TP) else numpy.nan, FP / (FP + TN) if (FP + TN) else numpy.nan, FP / (FP + TP) if (FP + TP) else numpy.nan, FN / (FN + TN) if (FN + TN) else numpy.nan, TP / (TP + FN + FP) if (TP + FN + FP) else numpy.nan, (TP + TN) / (TP + TN + FP + FN) if (TP + FP + FN) else numpy.nan, 2 * TP / (2 * TP + FP + FN) if (2 * TP + FP + FN) else numpy.nan, (TP / FP) / (FN / TN) if (FP and TN) else numpy.nana))))[0][1]
