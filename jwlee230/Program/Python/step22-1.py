"""
step22-1.py: Draw PCA from Beta-diversity TSV
"""
import argparse
import matplotlib
...
if __name__ == "__main__":
    ...
    pca_data = pandas.DataFrame(sklearn.decomposition.PCA(n_components=2, random_state=42).fit_transform(raw_data), columns=["PCA1", "PCA2"])

    for column in pca_data.columns:
        pca_data[column] = sklearn.preprocessing.scale(pca_data[column])

    pca_data["ShortStage"] = list(map(step00.change_ID_into_short_stage, list(raw_data.index)))
    pca_data["LongStage"] = list(map(step00.change_short_into_long, pca_data["ShortStage"]))
    print(pca_data)

    p_value = skbio.stats.distance.permanova(skbio.stats.distance.DistanceMatrix(raw_data, ids=list(raw_data.index)), grouping=list(pca_data["LongStage"]))["p-value"]
    print(p_value)
    ...
    seaborn.scatterplot(data=pca_data, x="PCA1", y="PCA2", hue="LongStage", ax=ax, legend="full", hue_order=step00.long_stage_order, palette=step00.color_stage_order, s=2000, edgecolor="none")

    for stage, color in zip(step00.long_stage_order, step00.color_stage_order):
        confidence_ellipse(pca_data.loc[(pca_data["LongStage"] == stage), "PCA1"], pca_data.loc[(pca_data["LongStage"] == stage), "PCA2"], ax, color=color, alpha=0.3)
